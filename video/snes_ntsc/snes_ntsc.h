...
snes_ntsc_t object. Can pass 0 for second parameter to use default setup. */
typedef struct snes_ntsc_t snes_ntsc_t;
void snes_ntsc_init(snes_ntsc_t*, snes_ntsc_setup_t const* setup);

/* Filter one or more rows of pixels. Input pixel format is set by SNES_NTSC_IN_FORMAT
and output RGB depth is set by NES_NTSC_OUT_DEPTH. Both default to 16-bit RGB.
In_row_width is the number of pixels to get to the next input row. Out_pitch
is the number of *bytes* to get to the next output row. */
void snes_ntsc_blit(snes_ntsc_t const*, SNES_NTSC_IN_T const* input,
    long in_row_width, int burst_phase, int in_width, int in_height,
    void* rgb_out, long out_pitch);

void snes_ntsc_blit_hires(snes_ntsc_t const*, SNES_NTSC_IN_T const* input,
    long in_row_width, int burst_phase, int in_width, int in_height,
    void* rgb_out, long out_pitch);
...
#define SNES_NTSC_HIRES_ROW(ntsc, burst, pixel1, pixel2, pixel3, pixel4, pixel5)                                \
    ...
    snes_ntsc_rgb_t const* kernel1 = SNES_NTSC_IN_FORMAT(ktable, snes_pixel1_);                                 \
    int const snes_pixel2_ = (pixel2);                                                                          \
    snes_ntsc_rgb_t const* kernel2 = SNES_NTSC_IN_FORMAT(ktable, snes_pixel2_);                                 \
    int const snes_pixel3_ = (pixel3);                                                                          \
    snes_ntsc_rgb_t const* kernel3 = SNES_NTSC_IN_FORMAT(ktable, snes_pixel3_);                                 \
    int const snes_pixel4_ = (pixel4);                                                                          \
    snes_ntsc_rgb_t const* kernel4 = SNES_NTSC_IN_FORMAT(ktable, snes_pixel4_);                                 \
    int const snes_pixel5_ = (pixel5);                                                                          \
    snes_ntsc_rgb_t const* kernel5 = SNES_NTSC_IN_FORMAT(ktable, snes_pixel5_);                                 \
...
#define SNES_NTSC_HIRES_OUT(x, rgb_out, bits)                                                                                                                                                                                                                                                                                                                 \
    {                                                                                                                                                                                                                                                                                                                                                         \
        snes_ntsc_rgb_t raw_ = kernel0[x] + kernel2[(x + 5) % 7 + 14] + kernel4[(x + 3) % 7 + 28] + kernelx0[(x + 7) % 7 + 7] + kernelx2[(x + 5) % 7 + 21] + kernelx4[(x + 3) % 7 + 35] + kernel1[(x + 6) % 7] + kernel3[(x + 4) % 7 + 14] + kernel5[(x + 2) % 7 + 28] + kernelx1[(x + 6) % 7 + 7] + kernelx3[(x + 4) % 7 + 21] + kernelx5[(x + 2) % 7 + 35]; \
...
struct snes_ntsc_t {
    snes_ntsc_rgb_t table[snes_ntsc_color_count][snes_ntsc_entry_size];
};
enum { snes_ntsc_burst_size = snes_ntsc_entry_size / snes_ntsc_burst_count };

#define SNES_NTSC_RGB16(ktable, n) \
    (snes_ntsc_rgb_t const*)(ktable + ((n & 0x001E) | (n >> 1 & 0x03E0) | (n >> 2 & 0x3C00)) * (snes_ntsc_entry_size / 2 * sizeof(snes_ntsc_rgb_t)))
...
#define SNES_NTSC_RGB_OUT_14_(x, rgb_out, bits, shift)                                                                                                                                  \
    {                                                                                                                                                                                   \
        snes_ntsc_rgb_t raw_ = kernel0[x] + kernel1[(x + 12) % 7 + 14] + kernel2[(x + 10) % 7 + 28] + kernelx0[(x + 7) % 14] + kernelx1[(x + 5) % 7 + 21] + kernelx2[(x + 3) % 7 + 35]; \
