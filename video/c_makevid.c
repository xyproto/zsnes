...
static void proc8x8(u2 const ax, u2 const dx, u1* const edi, u4 const layer, u1 const curcolor, u4 const bgptrb)
{
	...
	if (edi[eax] == 0)
	{
		...
		switch (curcolor)
		{
			case 1:  asm volatile("call %P0" :: "X" (cachetile2b), "a" (eax) : "cc", "memory", "ecx", "esi", "edi"); break;
			case 2:  asm volatile("call %P0" :: "X" (cachetile4b), "a" (eax) : "cc", "memory", "ecx", "esi", "edi"); break;
			default: asm volatile("call %P0" :: "X" (cachetile8b), "a" (eax) : "cc", "memory", "ecx", "esi", "edi"); break;
		}
	}
...
void procbackgrnd(u4 const layer)
{
	...
	if      (bgtilesz & curbgnum) proc16x16(y, x, edi, layer, mode, bgptrb);
	else if (bgmode == 5)         proc16x8( y, x, edi, layer, mode, bgptrb);
	else                          proc8x8(  y, x, edi, layer, mode, bgptrb);
}
