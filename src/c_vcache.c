...
static void cycleinputdevicemsg(void)
{
	...
	switch (device1)
	{
		case 0:  dev1 = "GAMEPAD"; break;
		default: dev1 = "MOUSE";   break;
	}

	char const* dev2;
	switch (device2)
	{
		default: dev2 = "GAMEPAD";      break;
		case 1:  dev2 = "MOUSE";        break;
		case 2:  dev2 = "SUPER SCOPE";  break;
		case 3:  dev2 = "1 JUSTIFIER";  break;
		case 4:  dev2 = "2 JUSTIFIERS"; break;
	}
...
static void cycleinputs(bool const input1, bool const input2)
{
	if (input2)
	{
		if (cycleinputdevice2() && input1) cycleinputdevice1();
		if (device2 == 2)
		{
			mousexloc = 128;
			mouseyloc = 112;
...
void cachevideo(void)
{
	...
	if (EMUPause != 1)
	{
		...
		if (RawDumpInProgress != 1)
		{
			if (FastFwdToggle == 0)
			{
				if (pressed[KeyFastFrwrd] & 1) goto fastfor;
			}
			else
			{
				if (TestKey2(KeyFastFrwrd)) FastForwardLock ^= 1;
...
fastfor:
		...
		if (FastFwdToggle == 0)
		{
			if (pressed[KeySlowDown] & 1) goto slowdwn;
		}
		else
		{
			if (TestKey2(KeySlowDown)) SlowDownLock ^= 1;
			if (SlowDownLock == 1)
			{
slowdwn:
...
throttleskip:
	...
	if (frameskip == 0)
	{
		while (t1cc < ax) Check60hz();
		t1cc -= ax;
		if (t1cc >= ax)
		{
			curblank = 0x40;
			if (++fskipped <= maxskip) goto nofrskip;
			t1cc     = 0;
			curblank = 0;
		}
		fskipped = 0;
...
nofrskip:
	...
	if (MouseDis != 1 && GUIRClick != 0 && device1 == 0 && device2 == 0)
	{
		...
		if (buttons & 0x02)
		{
			if (MousePRClick != 0) goto noclick;
			pressed[1] = 1;
		}
		MousePRClick = 0;
noclick:;
	...
	if (TestKey2(KeyBGDisble0)) ToggleLayer(0);
	if (TestKey2(KeyBGDisble1)) ToggleLayer(1);
	if (TestKey2(KeyBGDisble2)) ToggleLayer(2);
	if (TestKey2(KeyBGDisble3)) ToggleLayer(3);
	...
	if (TestKey2(KeyEmuSpeedDown) && EmuSpeed !=  0) --EmuSpeed;
	if (TestKey2(KeyEmuSpeedUp)   && EmuSpeed != 58) ++EmuSpeed;
	if (TestKey2(KeyResetSpeed))                     EmuSpeed = 29;
	...
	if (TestKey2(KeyRTRCycle))
	{
		char const* const msg =
			++MZTForceRTR == 3 ? MZTForceRTR = 0, "LOAD MZT MODE - OFF"    :
			MZTForceRTR   == 1 ?                  "LOAD MZT MODE - RECORD" :
			"LOAD MZT MODE - REPLAY";
		SetMessage(msg);
	}

	if (TestKey2(KeyExtraEnab1))  cycleinputs(true,  false);
	if (TestKey2(KeyExtraEnab2))  cycleinputs(false, true);
	if (TestKey2(KeyExtraRotate)) cycleinputs(true,  true);
	...
	if (TestKey2(KeyNewGfxSwt))
	{
		...
		for (u4* i = pal16bxcl; i != endof(pal16bxcl); ++i) *i = val;
		genfulladdtab();
	}
...
show_frameskip:
	...
	if (TestKey2(KeyDisplayBatt)) DisplayBatteryStatus();
...
show_gamma:;
	...
	if (TestKey2(KeyDisplayFPS) && frameskip == 0) FPSOn ^= 1;
