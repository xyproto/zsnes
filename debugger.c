...
WINDOW *openwindow(int nlines, int ncols, int begin_y, int begin_x, char *message) {
...
b:
	...
	case 'W': // break at signal (breakatsign)
	{
		...
		w = openwindow(3, 52, 11, 14, "   Waiting for Signal .... Press ESC to stop.");
		wrefresh(w);
		...
		do {
			execnextop();
		} while ((!((++numinst % 256) && (wgetch(w) == 27))) && (debstop3 != 1));
		debstop3 = 0;
		nodelay(w, FALSE);
...
void SPCbreakops(unsigned short addr) {
	...
	do {
		execnextop();
	} while ((!((++numinst % 256) && (wgetch(w) == 27))) && (spcPCRam != breakarea));
	nodelay(w, FALSE);
...
#define INDEX_RIGHT(addr, index)                            \
	...
	case 2: // $1234 : db+$1234
		wprintw(debugwin, "$%04x", *(unsigned short *)(instr + 1));
		wprintw(debugwin, "%5s[%02x%04x] ", padding, GETXB(), *(unsigned short *)(instr + 1));
		break;
	...
	case 4: // $12 : $12+d
		wprintw(debugwin, "$%02x%7s[%02x%04x] ", instr[1], padding, 0, instr[1] + xd);
		break;
	...
	case 26: // #$12,#$1234 (X-flag)
		if (xp & 0x10) {
			...
			wprintw(debugwin, "#$%04x%13s", *(unsigned short *)(instr + 1), padding);
		}
		break;
...
void out65816() {
	...
	wprintw(debugwin, "A:%04x X:%04x Y:%04x S:%04x DB:%02x D:%04x P:%02x %c", xa, xx, xy, xs, xdb, xd, xp, (xe == 1) ? 'E' : 'e');
}
...
void nextspcopcode() {
	...
	wprintw(debugwin, "N%cO%cD%c?%cH%cI%cZ%cC%c", (spcNZ & 0x80) ? '+' : '-', (spcP & 0x40) ? '+' : '-', (spcP & 0x20) ? '+' : '-', (spcP & 0x10) ? '+' : '-', (spcP & 0x08) ? '+' : '-', (spcP & 0x04) ? '+' : '-', (spcP & 0x02) ? '+' : '-', (spcP & 0x01) ? '+' : '-');
