	...
	*/
	...
	if (MSUEnable) {
		...
		if (method == csm_load_zst_new) {
			// if the track changed we have to make sure
			copy_func(&buffer, &MSU_Track_Previous, sizeof(MSU_Track_Previous));
			if (MSU_Track != MSU_Track_Previous) {
				MSU_Track = MSU_Track_Previous;
				MSU1LoadNewTrack();
			}
		} else {
			copy_func(&buffer, &MSU_Track, sizeof(MSU_Track));
		}
	}
