...
template <int aNumMoveBits, UINT32 m_NumBitLevels> class CBitTreeDecoder {
...
template <int aNumMoveBits> class CReverseBitTreeDecoder2 {
...
public:
    ...
    UINT32 Decode(CMyRangeDecoder* aRangeDecoder)
    {
        ...
        for (UINT32 aBitIndex = 0; aBitIndex < m_NumBitLevels; aBitIndex++) {
            ...
            RC_GETBIT2(aNumMoveBits, m_Models[aModelIndex].m_Probability, aModelIndex, ;
                       , aSymbol |= (1 << aBitIndex))
        }
        RC_FLUSH_VAR
        return aSymbol;
...
template <int aNumMoveBits, UINT32 m_NumBitLevels> class CReverseBitTreeDecoder {
    CMyBitDecoder<aNumMoveBits> m_Models[1 << m_NumBitLevels];
