...
namespace NArithmetic {
    ...
    public:
        ...
        {
            /*
            m_Probability -= (m_Probability + ((aSymbol - 1) & ((1 << aNumMoveBits) - 1))) >>
            aNumMoveBits; m_Probability += (1 - aSymbol) << (kNumBitModelTotalBits - aNumMoveBits);
            */
            if (aSymbol == 0)
    ...
    template <int aNumMoveBits> class CBitDecoder : public CBitModel<aNumMoveBits> {
    public:
        UINT32 Decode(CRangeDecoder* aRangeDecoder)
        {
            ...
            if (aRangeDecoder->m_Code < aNewBound) {
                ...
                if (aRangeDecoder->m_Range < kTopValue) {
                    aRangeDecoder->m_Code
                        = (aRangeDecoder->m_Code << 8) | aRangeDecoder->m_Stream.ReadByte();
