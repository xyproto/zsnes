...
void jma_open::retrieve_file_block() throw(jma_errors)
{
    ...
    if (!byte) // If file block is compressed
    {
        ...
        if (!decompress_lzma_7z(
                compressed_data, compressed_size, decompressed_data, file_block_size)) {
    ...
    while (file_block_size
        >= 2 + 1 + UINT_SIZE * 2 + USHORT_SIZE * 2) // This does allow for a gap, but that's okay
    {
        ...
        file_block_size -= file_info.name.length() + file_info.comment.length() + 2 + UINT_SIZE * 2
            + USHORT_SIZE * 2;
    }
}
...
vector<unsigned char*> jma_open::get_all_files(unsigned char* buffer) throw(jma_errors)
{
    ...
    if (chunk_size) {
        ...
        for (size_t remaining_size = size; remaining_size; remaining_size -= chunk_size) {
            ...
            if (CRC32lib::CRC32(compressed_buffer, compressed_size)
                != charp_to_uint(int4_buffer)) {
    ...
    } else // Solidly compressed JMA
    {
        ...
        // Setup access methods for decompression
        ISequentialInStream_Istream compressed_data(stream);
        ISequentialOutStream_Array decompressed_data(
            reinterpret_cast<char*>(decompressed_buffer), size);
...
bool jma_open::is_solid() { return (chunk_size ? false : true); }
