...
DWORD LastUsedPos = 0;
DWORD CurMode = ~0;
void initDirectDraw() { InitDirectDraw(); }
void reInitSound() { ReInitSound(); }
}
...
void DrawScreen()
{
    if (FullScreen == 1) {
        if (TripleBufferWin == 1 || KitchenSync == 1) {
            if (DD_BackBuffer->Blt(&rcWindow, DD_CFB, &BlitArea, DDBLT_WAIT, NULL)
                == DDERR_SURFACELOST)
                DD_Primary->Restore();
        ...
        DD_Primary->Blt(
            &rcWindow, AltSurface == 0 ? DD_CFB : DD_CFB16, &BlitArea, DDBLT_WAIT, NULL);
    }
}
    ...
    case WM_KEYDOWN: // sent when user presses a key
        if (!((CurKeyPos + 1 == CurKeyReadPos)
            ...
            if (((wParam >= 'A') && (wParam <= 'Z')) || ((wParam >= 'a') && (wParam <= 'z'))
                || (wParam == 27) || (wParam == 32) || (wParam == 8) || (wParam == 13)
                || (wParam == 9)) {
                accept = true;
                vkeyval = wParam;
            }
...
BOOL InitSound()
{
    ...
    if (DS_OK == lpDirectSound->CreateSoundBuffer(&dsbd, &lpPrimaryBuffer, NULL)) {
        if (!UsePrimaryBuffer) {
            if (DS_OK
                == lpPrimaryBuffer->QueryInterface(
                    IID_IDirectSoundBuffer8, (LPVOID*)&lpSoundBuffer)) {
...
BOOL FAR PASCAL InitJoystickInput(LPCDIDEVICEINSTANCE pdinst, LPVOID pvRef)
{
    ...
    if (JoystickInput[CurrentJoy]->SetCooperativeLevel(
            hMainWindow, DISCL_NONEXCLUSIVE | DISCL_FOREGROUND)
        != DI_OK) {
        JoystickInput[CurrentJoy]->Release();
        return DIENUM_CONTINUE;
    }
...
void DInputError()
{
    ...
    sprintf(message1,
        "Error initializing DirectInput\nYou may need to install DirectX 8.0a or higher located "
        "at www.microsoft.com/directx \0");
...
bool InitInput()
{
    ...
    if (FAILED(hr = pDirectInput8Create(
                   hInst, DIRECTINPUT_VERSION, IID_IDirectInput8A, (void**)&DInput, NULL))) {
        sprintf(message1,
            "Error initializing DirectInput\nYou may need to install DirectX 8.0a or higher "
            "located at www.microsoft.com/directx \0");
        MessageBox(NULL, message1, "DirectInput Error", MB_ICONERROR);
...
int InitDirectDraw()
{
    ...
    for (i = 0; i < 32; i++)
        for (j = 0; j < 64; j++)
            for (k = 0; k < 32; k++) {
                ...
                u = 128 + ((r - b) >> 2);
                v = 128 + ((-r + 2 * g - b) >> 3);
                *(((unsigned int*)RGBtoYUVPtr) + (i << 11) + (j << 5) + k)
                    = (Y << 16) + (u << 8) + v;
            }
    ...
    if (FullScreen == 1) {
        if (lpDD->SetCooperativeLevel(
                hMainWindow, DDSCL_FULLSCREEN | DDSCL_EXCLUSIVE | DDSCL_ALLOWREBOOT)
            != DD_OK) {
    ...
    if (DD_Primary->GetPixelFormat(&format) != DD_OK) {
        MessageBox(
            NULL, "IDirectDrawSurface7::GetPixelFormat failed.", "DirectDraw Error", MB_ICONERROR);
        return FALSE;
    }
    ...
    if (BitDepth == 24) {
        MessageBox(NULL,
            "ZSNESw does not support 24bit color.\nPlease change your resolution to either 16bit "
            "or 32bit color",
            "Error", MB_OK);
        exit(0);
    }
    ...
    if (BitDepth == 32) {
        ...
        if (lpDD->CreateSurface(&ddsd2, &DD_CFB16, NULL) != DD_OK) {
            MessageBox(
                NULL, "IDirectDraw7::CreateSurface failed.", "DirectDraw Error", MB_ICONERROR);
            return FALSE;
        }
...
void Stop36HZ(void) { T36HZEnabled = 0; }
...
void initwinvideo(void)
{
    ...
    if (!FirstVid) {
        if (X < 0)
            X = 0;
        if (X > (int)(GetSystemMetrics(SM_CXSCREEN) - WindowWidth))
            X = (GetSystemMetrics(SM_CXSCREEN) - WindowWidth);
        if (Y < 0)
            Y = 0;
        if (Y > (int)(GetSystemMetrics(SM_CYSCREEN) - WindowHeight))
            Y = (GetSystemMetrics(SM_CYSCREEN) - WindowHeight);
        ...
        if (FullScreen == 1) {
            if (HQMode && !DSMode) {
                int marginx
                    = (rcWindow.right - rcWindow.left - BlitArea.right + BlitArea.left) / 2;
                int marginy
                    = (rcWindow.bottom - rcWindow.top - BlitArea.bottom + BlitArea.top) / 2;
                if (marginx > 0) {
                    rcWindow.left += marginx;
                    rcWindow.right -= marginx;
                }
        ...
        hMainWindow = CreateWindow("ZSNESWIN", WinName, WS_VISIBLE | WS_POPUP, X,
            Y, // WS_OVERLAPPED "ZSNESWIN"
...
void UpdateVFrame(void)
{
    ...
    while (DataNeeded > 0) {
        ...
        if (!UsePrimaryBuffer) {
            if (DS_OK
                != lpSoundBuffer->Lock(
                    LastUsedPos, SPCSize * 2, &lpvPtr1, &dwBytes1, &lpvPtr2, &dwBytes2, 0)) {
                return;
            }
        } else {
            if (DS_OK
                != lpPrimaryBuffer->Lock(
                    LastUsedPos, SPCSize * 2, &lpvPtr1, &dwBytes1, &lpvPtr2, &dwBytes2, 0)) {
                return;
            }
        }
...
void drawscreenwin(void)
{
    ...
    if (HQMode == 0) {
        ...
        default: {
            UnlockSurface();
            MessageBox(
                NULL, "Mode only available in 16 and 32 bit color", "DDRAW Error", MB_ICONERROR);
            cvidmode = 2;
            initwinvideo();
            Sleep(1000);
            drawscreenwin();
...
int GetMouseX(void)
{
    ...
    if (MouseX > MouseMaxX) {
        ...
        if (TrapMouseCursor == 1) {
            if (abs((int)MouseMoveX) > 10 && T36HZEnabled == 1 && FullScreen == 0
...
int GetMouseY(void)
{
    ...
    if (MouseY > MouseMaxY) {
        ...
        if (TrapMouseCursor == 1) {
            if (abs((int)MouseMoveY) > 10 && T36HZEnabled == 1 && FullScreen == 0
                && MouseButtonPressed == 0) {
                MouseInput->Unacquire();
...
void SetMouseMinX(int MinX) { MouseMinX = (float)MinX; }

void SetMouseMaxX(int MaxX) { MouseMaxX = (float)MaxX; }

void SetMouseMinY(int MinY) { MouseMinY = (float)MinY; }

void SetMouseMaxY(int MaxY) { MouseMaxY = (float)MaxY; }

void SetMouseX(int X) { MouseX = (float)X; }

void SetMouseY(int Y) { MouseY = (float)Y; }
