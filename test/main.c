...
static void test_natify_slashes(void)
{
    ...
    { char s[] = "a\\b\\c";       natify_slashes(s); ZT_CHECK_STR(s, "a/b/c"); }
    { char s[] = "a/b/c";         natify_slashes(s); ZT_CHECK_STR(s, "a/b/c"); }
    { char s[] = "";               natify_slashes(s); ZT_CHECK_STR(s, "");      }
    { char s[] = "\\";             natify_slashes(s); ZT_CHECK_STR(s, "/");     }
    { char s[] = "foo\\bar\\baz";  natify_slashes(s); ZT_CHECK_STR(s, "foo/bar/baz"); }
    { char s[] = "a\\b/c\\d";      natify_slashes(s); ZT_CHECK_STR(s, "a/b/c/d"); }
}
...
static void test_strcutslash(void)
{
    ...
    { char s[] = "/foo/bar/";  strcutslash(s); ZT_CHECK_STR(s, "/foo/bar"); }
    { char s[] = "/foo/bar";   strcutslash(s); ZT_CHECK_STR(s, "/foo/bar"); }
    { char s[] = "/";          strcutslash(s); ZT_CHECK_STR(s, "");         }
    { char s[] = "foo/";       strcutslash(s); ZT_CHECK_STR(s, "foo");      }
    { char s[] = "foo\\";      strcutslash(s); ZT_CHECK_STR(s, "foo");      }
}
...
static void test_strcatslash(void)
{
    ...
    { char s[32] = "/foo/bar";  strcatslash(s); ZT_CHECK_STR(s, "/foo/bar/"); }
    { char s[32] = "/foo/bar/"; strcatslash(s); ZT_CHECK_STR(s, "/foo/bar/"); }
    { char s[32] = "foo";       strcatslash(s); ZT_CHECK_STR(s, "foo/");      }
...
static void test_setextension(void)
{
    ...
    { char s[32] = "file.txt";    setextension(s, "c");   ZT_CHECK_STR(s, "file.c");   }
    { char s[32] = "file.txt";    setextension(s, "zsn"); ZT_CHECK_STR(s, "file.zsn"); }
    { char s[32] = "file";        setextension(s, "txt"); ZT_CHECK_STR(s, "file.txt"); }
    { char s[32] = "a.b.c";       setextension(s, "d");   ZT_CHECK_STR(s, "a.b.d");   }
    { char s[32] = "ROM";         setextension(s, "sfc"); ZT_CHECK_STR(s, "ROM.sfc");  }
    { char s[32] = "save.zst";    setextension(s, "zst"); ZT_CHECK_STR(s, "save.zst"); }
}
...
static void test_strdirname(void)
{
    ...
    { char s[64] = "/foo/bar/baz";  strdirname(s); ZT_CHECK_STR(s, "/foo/bar"); }
    { char s[64] = "/foo/bar/baz/"; strdirname(s); ZT_CHECK_STR(s, "/foo/bar"); }
    { char s[64] = "/foo";          strdirname(s); ZT_CHECK_STR(s, "/");        }
    { char s[64] = "/";             strdirname(s); ZT_CHECK_STR(s, "/");        }
    /* no slash – element with no directory component is left unchanged */
    { char s[64] = "baz";           strdirname(s); ZT_CHECK_STR(s, "baz");      }
...
static void test_strbasename(void)
{
    ...
    { char s[64] = "/foo/bar/baz"; strbasename(s); ZT_CHECK_STR(s, "baz");     }
    { char s[64] = "/baz";         strbasename(s); ZT_CHECK_STR(s, "baz");     }
    { char s[64] = "baz";          strbasename(s); ZT_CHECK_STR(s, "baz");     }
    { char s[64] = "foo\\bar";     strbasename(s); ZT_CHECK_STR(s, "bar");     }
}
...
static void test_strdupcat(void)
{
    ...
    r = strdupcat("hello", "world"); ZT_CHECK(r != NULL); ZT_CHECK_STR(r, "helloworld"); free(r);
    r = strdupcat("",      "abc");   ZT_CHECK(r != NULL); ZT_CHECK_STR(r, "abc");        free(r);
    r = strdupcat("abc",   "");      ZT_CHECK(r != NULL); ZT_CHECK_STR(r, "abc");        free(r);
    r = strdupcat("",      "");      ZT_CHECK(r != NULL); ZT_CHECK_STR(r, "");           free(r);
    r = strdupcat("/foo/", "bar");   ZT_CHECK(r != NULL); ZT_CHECK_STR(r, "/foo/bar");   free(r);
    /* result length */
    r = strdupcat("ab", "cd");       ZT_CHECK(r && strlen(r) == 4);                      free(r);
}
