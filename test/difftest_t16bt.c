...
extern u1 a16x16xinc, a16x16yinc;
extern u2 yadd, yflipadd, curypos;
...
void asm_draw16x1616bt(void);
void cur_draw16x1616bt(void);
    ...
    snapshot* const out)
{
    ...
    /* The routine writes all four of these; poison them first, or a port that
       ...
       value behind. */
    temp = bshifter = a16x16xinc = a16x16yinc = 0xA5;
...
int main(void)
{
    ...
    DT_MAIN(20260811, 30000)
    {
        ...
        run(asm_draw16x1616bt, &in, &x);
        run(cur_draw16x1616bt, &in, &y);
        ...
        if (dt_bad && DT_SHOW()) {
            printf("  ^ route=%d mosaic=%u winon=%u hofs=%u xinc=%u "
                   "cache=%u off=%u\n",
                route, curmosaicsz, winon, hofs, a16x16xinc, cache, off);
        }
    }
    printf("  winon=%ld mosaic=%ld plain=%ld domosaic=%ld yinc=%ld\n", cov[0],
        cov[1], cov[2], cov[3], cov[4]);
    DT_DONE("makev16t draw16x1616bt / draw16x1616btwinon");
}
