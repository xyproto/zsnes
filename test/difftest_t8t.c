 ...
 * behind one entry point, so one comparison covers all six. Both sides are
 ...
 * draw8x816toffset and the bgmode 5 jump into draw16x816t (both rewritten by
...
int main(void)
{
    ...
    DT_MAIN(20260811, 30000)
    {
        ...
        run(asm_draw8x816t, &in, &x);
        run(cur_draw8x816t, &in, &y);
