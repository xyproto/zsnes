 ...
 *   c_DSP2Read8b, c_DSP2Read16b, c_DSP2Write8b, c_DSP2Write16b
...
/* ======================================================================== */
/* c_DSP2Read16b / c_DSP2Write16b                                               */
/* ======================================================================== */

static void test_read16b(void)
{
    ZT_SECTION("c_DSP2Read16b: always returns 0");
    ZT_CHECK(c_DSP2Read16b(0x0000) == 0);
    ZT_CHECK(c_DSP2Read16b(0x8000) == 0);
    ZT_CHECK(c_DSP2Read16b(0xFFFF) == 0);
}

static void test_write16b(void)
{
    ZT_SECTION("c_DSP2Write16b: no-op, does not crash");
    c_DSP2Write16b(0x8000, 0x1234);
    c_DSP2Write16b(0x0000, 0xABCD);
    ZT_CHECK(1);
}
...
{
    ZT_SECTION("c_DSP2Read8b: HALT flag set → returns 0");
    RESET_DSP2();
    dsp2state = DSP2F_HALT;
    dsp2buffer[0] = 0x42;
    ZT_CHECK(c_DSP2Read8b(0x8000) == 0);
}

static void test_read8b_addr_validation(void)
{
    ZT_SECTION("c_DSP2Read8b: bit-15 required, bits 14-12 must be clear → else 0");
    RESET_DSP2();
    dsp2buffer[0] = 0x55;

    /* bit 15 not set */
    ZT_CHECK(c_DSP2Read8b(0x0000) == 0);
    ZT_CHECK(c_DSP2Read8b(0x7FFF) == 0);

    /* bit 15 set but bits 14-12 not all clear */
    ZT_CHECK(c_DSP2Read8b(0x9000) == 0); /* bit 12 set */
    ZT_CHECK(c_DSP2Read8b(0xA000) == 0); /* bit 13 set */
    ZT_CHECK(c_DSP2Read8b(0xC000) == 0); /* bit 14 set */

    /* valid: bit 15 set, bits 14-12 clear */
    ZT_CHECK(c_DSP2Read8b(0x8000) == 0x55);
}

static void test_read8b_index(void)
{
    ZT_SECTION("c_DSP2Read8b: index = addr & 0xFF into dsp2buffer");
    RESET_DSP2();
    dsp2buffer[0x00] = 0x11;
    dsp2buffer[0x42] = 0x42;
    dsp2buffer[0xFF] = 0xFF;

    ZT_CHECK(c_DSP2Read8b(0x8000) == 0x11); /* 0x8000 & 0xFF = 0x00 */
    ZT_CHECK(c_DSP2Read8b(0x8042) == 0x42);
    ZT_CHECK(c_DSP2Read8b(0x80FF) == 0xFF);
    /* bits 11-8 don't affect index (only 7-0 kept) */
    ZT_CHECK(c_DSP2Read8b(0x8F42) == 0x42); /* 0x8F42 & 0xFF = 0x42 */
}

static void test_read8b_auto_shift(void)
{
    ZT_SECTION("c_DSP2Read8b: AUTO_BUFFER_SHIFT → sar dword[dsp2buffer], 8");
    RESET_DSP2();
...
{
    ZT_SECTION("c_DSP2Read8b: AUTO_BUFFER_SHIFT is arithmetic (sign-extends)");
    RESET_DSP2();
    dsp2state = DSP2F_AUTO_BUFFER_SHIFT;
...
/* ======================================================================== */
/* c_DSP2Write8b — infrastructure                                             */
/* ======================================================================== */

static void test_write8b_halt(void)
{
    ZT_SECTION("c_DSP2Write8b: HALT flag set → no-op");
    RESET_DSP2();
    dsp2state = DSP2F_HALT;
    put_entry(0, 0x04, 0, 0x0000);
    dsp2buffer[0] = 0xAA;
    c_DSP2Write8b(0x0000, 0xFF);
...
{
    ZT_SECTION("c_DSP2Write8b: addr mismatch → sets HALT flag");
    RESET_DSP2();
    put_entry(0, 0x0B, 0, 0x1234); /* queue expects addr 0x1234 */
    c_DSP2Write8b(0x5678, 0x42); /* write to wrong address */
    ZT_CHECK(dsp2state & DSP2F_HALT);
}

static void test_write8b_addr_match(void)
{
    ZT_SECTION("c_DSP2Write8b: addr match → command executes, cursor advances");
    RESET_DSP2();
    put_entry(0, 0x0B, 7, 0x0001); /* w0B: write input to buffer[7], addr=1 */
    c_DSP2Write8b(0x0001, 0xAB);
    ZT_CHECK(!(dsp2state & DSP2F_HALT));
...
{
    ZT_SECTION("c_DSP2Write8b: NO_ADDR_CHK bypasses address validation");
    RESET_DSP2();
    dsp2state = DSP2F_NO_ADDR_CHK;
    put_entry(0, 0x0B, 3, 0x1234); /* queue expects 0x1234 */
    c_DSP2Write8b(0xFFFF, 0x99); /* completely different address */
    ZT_CHECK(!(dsp2state & DSP2F_HALT));
...
static void test_w02(void)
{
    ...
    c_DSP2Write8b(0x0000, 0xA5); /* low nibble = 5 */
    ...
    RESET_DSP2();
    put_entry(0, 0x02, 0, 0x0000);
    c_DSP2Write8b(0x0000, 0xF0); /* low nibble = 0 */
...
static void test_w07(void)
{
    ...
    c_DSP2Write8b(0x0000, 0xAB); /* rol 4: 0xBA */
    ZT_CHECK(dsp2buffer[10] == 0xBA);

    RESET_DSP2();
    put_entry(0, 0x07, 0, 0x0000);
    c_DSP2Write8b(0x0000, 0x12);
...
static void test_w05(void)
{
    ...
    c_DSP2Write8b(0x0000, 0x33); /* both nibbles match key */
...
static void test_w00_unknown_subcmd(void)
{
    ...
    RESET_DSP2();
    put_entry(0, 0x00, 0, 0x8000);
    c_DSP2Write8b(0x8000, 0x02); /* 0x02 not in w00 dispatch */
    ZT_CHECK(dsp2state & DSP2F_HALT);
}
...
static void test_w00p02_chain(void)
{
    ...
    c_DSP2Write8b(0x8000, 0x03); /* executes slot 0 = w00, subcmd=03 */
    ZT_CHECK(!(dsp2state & DSP2F_HALT));
    ZT_CHECK(dsp2enforcerReaderCursor == 1);

    /* Now the writer has pushed [cmd=2] and [cmd=0] into slots 1 and 2.
     * Reader is at 1. */
    c_DSP2Write8b(0x8000, 0xC7); /* executes slot 1 = w02, input=0xC7 */
    ZT_CHECK(!(dsp2state & DSP2F_HALT));
...
static void test_w06_w07_chain(void)
{
    ...
    RESET_DSP2();
    put_entry(0, 0x06, 0, 0x0000);
    c_DSP2Write8b(0x0000, 2); /* queues: w07@0x8000(p1=1), w07@0x8001(p1=0), w00@0x8000 */

    /* first queued w07: param1=1, addr=0x8000 */
    c_DSP2Write8b(0x8000, 0xAB); /* buffer[1] = rol(0xAB,4) = 0xBA */
    ZT_CHECK(!(dsp2state & DSP2F_HALT));
    ZT_CHECK(dsp2buffer[1] == 0xBA);

    /* second queued w07: param1=0, addr=0x8001 */
    c_DSP2Write8b(0x8001, 0x12); /* buffer[0] = rol(0x12,4) = 0x21 */
    ZT_CHECK(dsp2buffer[0] == 0x21);
}
...
static void test_w08_multiply_at_idx3(void)
{
    ...
    RESET_DSP2();
    put_entry(0, 0x08, 0, 0x8000);
    c_DSP2Write8b(0x8000, 3); /* buffer[0]=3 */
    put_entry(1, 0x08, 2, 0x8000);
    c_DSP2Write8b(0x8000, 5); /* buffer[2]=5 */
    put_entry(2, 0x08, 3, 0x8000);
    c_DSP2Write8b(0x8000, 0); /* triggers 3×5=15 */
    ZT_CHECK(!(dsp2state & DSP2F_HALT));
    ZT_CHECK(dsp2state & DSP2F_AUTO_BUFFER_SHIFT);
...
static void test_w08_multiply_large(void)
{
    ...
    RESET_DSP2();
    put_entry(0, 0x08, 0, 0x8000);
    c_DSP2Write8b(0x8000, 255);
    put_entry(1, 0x08, 2, 0x8000);
    c_DSP2Write8b(0x8000, 255);
    put_entry(2, 0x08, 3, 0x8000);
    c_DSP2Write8b(0x8000, 0);
...
static void test_w09_halt_on_zero(void)
{
    ...
    c_DSP2Write8b(0x0000, 0x00); /* sar 0→0 */
    ZT_CHECK(dsp2state & DSP2F_HALT);

    RESET_DSP2();
    put_entry(0, 0x09, 0, 0x0000);
    c_DSP2Write8b(0x0000, 0x01); /* sar 1→0 (signed) */
    ZT_CHECK(dsp2state & DSP2F_HALT);
}
...
static void test_w09_sets_size_queues_w0a(void)
{
    ...
    put_entry(0, 0x09, 0, 0x0000);
    uint32_t wc = dsp2enforcerWriterCursor;
    c_DSP2Write8b(0x0000, 0x04); /* sar 4→2 */
    ZT_CHECK(!(dsp2state & DSP2F_HALT));
...
static void test_w09_w0a_chain(void)
{
    ...
    RESET_DSP2();
    put_entry(0, 0x09, 0, 0x0000);
    c_DSP2Write8b(0x0000, 0x08); /* sar 8→4 → origSize=4; queues w0A */

    /* w0A is now at reader slot 1; the queued addr is 0x8000 */
    c_DSP2Write8b(0x8000, 0x04); /* sar 4→2 → newSize=2; queues 4 w0B + w00 */
    ZT_CHECK(!(dsp2state & DSP2F_HALT));
...
static void test_multiply_chain_via_w00p09(void)
{
    ...
    RESET_DSP2();
    put_entry(0, 0x00, 0, 0x8000);
    c_DSP2Write8b(0x8000, 0x09); /* queues 4×w08 + w00 at slots 1..5 */
    c_DSP2Write8b(0x8000, 4); /* w08 param1=0 → buffer[0]=4        */
    c_DSP2Write8b(0x8000, 0); /* w08 param1=1 → buffer[1]=0        */
    c_DSP2Write8b(0x8000, 7); /* w08 param1=2 → buffer[2]=7        */
    c_DSP2Write8b(0x8000, 0); /* w08 param1=3 → 4×7=28             */
    ZT_CHECK(!(dsp2state & DSP2F_HALT));
    ZT_CHECK(dsp2state & DSP2F_AUTO_BUFFER_SHIFT);
...
static void test_multiply_then_read(void)
{
    ...
    put_entry(0, 0x00, 0, 0x8000);
    c_DSP2Write8b(0x8000, 0x09);
    c_DSP2Write8b(0x8000, 4); /* buffer[0]=4  */
    c_DSP2Write8b(0x8000, 0); /* buffer[1]=0  */
    c_DSP2Write8b(0x8000, 255); /* buffer[2]=255 */
    c_DSP2Write8b(0x8000, 0); /* 4×255=1020=0x000003FC */
    ZT_CHECK(dsp2buffer[0] == 0xFC);
    ZT_CHECK(dsp2buffer[1] == 0x03);
    /* Read8b consumes via arithmetic shift: 0x000003FC→0x000003, etc. */
    ZT_CHECK(c_DSP2Read8b(0x8000) == 0xFC); /* buffer[0] before shift */
    ZT_CHECK(dsp2buffer[0] == 0x03); /* 0x3FC >> 8 = 0x3       */
    ZT_CHECK(dsp2buffer[1] == 0x00);
    ZT_CHECK(c_DSP2Read8b(0x8000) == 0x03);
    ZT_CHECK(dsp2buffer[0] == 0x00);
    ZT_CHECK(c_DSP2Read8b(0x8000) == 0x00); /* product fully drained   */
}
