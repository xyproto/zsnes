 ...
 *
 * draw16x816t and everything reachable from it sit behind one entry point, so
 * one comparison covers all twelve. Both sides are
 ...
 * res512switch picks which half of each 16-pixel tile the line takes, so it is
 * driven both ways.
 */
#include <stdint.h>
#include <stdio.h>
...
extern u1 osm2dis, scaddtype, coadder16, res512switch;
...
void asm_draw16x816t(void);
void cur_draw16x816t(void);
    ...
    snapshot* const out)
{
    ...
    /* An output too: restore it, or the C run inherits the mark the asm run
       left and a port that never writes it still agrees. */
    memcpy(hirestiledat, in->hires, 256);
    ...
    memcpy(out->hires, hirestiledat, 256);
    out->mem[0] = temp;
    out->mem[1] = bshifter;
    out->mem[2] = drawn;
    out->ex = ex;
...
int main(void)
{
    ...
    DT_MAIN(20260811, 30000)
    {
        ...
        /* Which half of each 16-pixel tile this line takes. */
        res512switch = (u1)(dt_mod(2) ? dt_mod(2) : dt_u32());
        curbgpr = (u1)(dt_mod(2) ? 0x20u : 0);
        /* `mov al,[curypos]` is a byte read of a u2 that runs past 255. */
        curypos = (u2)dt_mod(0x200);
        dt_fill(in.hires, 256);
        ...
        run(asm_draw16x816t, &in, &x);
        run(cur_draw16x816t, &in, &y);
        ...
        DT_MEM("hirestiledat", x.hires, y.hires, sizeof x.hires);
        ...
        if (dt_bad && DT_SHOW()) {
            u4 base = 64u - hofs * 2u;
            printf("  esi=%u tiles:", base);
            for (u4 j = 0; j < 10; j++) {
                printf(" %04x/%04x", *(u2*)(x.vid + base + j * 2),
                    *(u2*)(y.vid + base + j * 2));
            }
            printf("\n  map0=%04x curbgpr=%02x bshifter=%u\n",
                *(u2*)(u1*)(uintptr_t)in.reg[5], curbgpr, bshifter);
            printf("  ^ route=%d half=%u mosaic=%u winon=%u hofs=%u "
                   "scaddtype=%02x scrnon=%04x cache=%u off=%u\n",
                route, res512switch, curmosaicsz, winon, hofs, scaddtype,
                scrnon, cache, off);
        }
    }
    ...
    DT_DONE("makev16t 16x8 hi-res colour-maths drawers");
}
