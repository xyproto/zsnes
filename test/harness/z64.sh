...
if [ -z "$REUSE" ]; then
    ...
    make -C "$ROOT" WITH_DEBUG_HOOKS=1 linux_i686 >"$OUT/build32.log" 2>&1 || { echo "32-bit BUILD FAILED"; exit 1; }
    cp "$ROOT/zsnes" "$OUT/zsnes32"
    echo "=== building 64-bit ==="
    make -C "$ROOT" clean >/dev/null 2>&1
    make -C "$ROOT" WITH_DEBUG_HOOKS=1 linux_x86_64 >"$OUT/build64.log" 2>&1 || { echo "64-bit BUILD FAILED"; exit 1; }
