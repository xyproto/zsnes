...
void loadFile(char *filename) {
	...
	for (;;) {
		...
		if ((uint32_t)stat_results.st_size <= MAXROMSPACE + 512 - curromspace) {
			...
			if (!fp) {
				printf("Windows failed to read %s%s: error %d\n", ZRomPath, filename, errno);
				return;
			}
...
void clearmem(void) {
	...
	memset(romdata, 0xFF, MAXROMSPACE);
	clearmem2();
}
...
uint32_t showinfogui(void) {
	...
	strcpy(CSStatus2, "SPECIAL: No               CHKSUM:       ");
	...
	if ((ROM[i] == (Checksumvalue & 0xFF)) && (ROM[i + 1] == (Checksumvalue >> 8))) {
		memcpy(CSStatus2 + 34, "OK  ", 4);
	} else {
		memcpy(CSStatus2 + 34, "FAIL", 4);
		if (!IPSPatched) {
...
void SetIRQVectors() { // get vectors (NMI & reset)
	uint8_t *ROM = romdata;
