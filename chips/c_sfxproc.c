...
void initsfxregsr(void)
{
	...
	eop** i = &REGPTR(0x3100);
	do *i = cacheregr; while (++i != &REGPTR(0x3300)); // XXX code and comment disagree: 0x31FF vs. 0x32FF
}
...
void initsfxregsw(void)
{
	...
	eop** i = &REGPTW(0x3100);
	do *i = cacheregw; while (++i != &REGPTW(0x3300)); // XXX code and comment disagree: 0x31FF vs. 0x32FF
}
