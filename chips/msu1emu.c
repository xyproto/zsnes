...
#endif
		fseek(MSUBinary, 0, SEEK_END);
		filelen = ftell(MSUBinary);
		rewind(MSUBinary);
...
// Function to load MSU_Track into TRACK_DATA
void MSU1LoadNewTrack() {
#ifdef DEBUG
	...
	if (TrackFileReader) {
		fseek(TrackFileReader, 0, SEEK_END);
		filelen = ftell(TrackFileReader);
	...
	if ((MSU_StatusRead & 0x10) && MSU_Track_Length > 0) {
		...
		for (; start < end; start++) {
			...
			if (MSU_Track_Position < MSU_Track_Length) {
				*start += (TRACK_DATA[MSU_Track_Position] * MSU_MusicVolume) / 0xC0;

				// Stereo Mixer
				if (StereoSound) {
					*start++;
					*start += (TRACK_DATA[MSU_Track_Position + 1] * MSU_MusicVolume) / 0xC0;
				}
