...
void DrawWater(void) {
	...
	if (mode & 1) {
		int x, y;
		x = (SCRW / 2) + ((((FSin((xang * 65) >> 8) >> 8) * (FSin((xang * 349) >> 8) >> 8)) * ((SCRW - 8) / 2)) >> 16);

		y = (SCRH / 2) + ((((FSin((yang * 377) >> 8) >> 8) * (FSin((yang * 84) >> 8) >> 8)) * ((SCRH - 8) / 2)) >> 16);
		xang += 13;
		yang += 12;
...
void CalcWater(int *nptr, int *optr, int density) {
	...
	for (y = (SCRH - 1) * SCRW; count < y; count += 2) {
		for (x = count + SCRW - 2; count < x; count++) {
			newh = ((optr[count + SCRW] + optr[count - SCRW] + optr[count + 1] + optr[count - 1] + optr[count - SCRW - 1] + optr[count - SCRW + 1] + optr[count + SCRW - 1] + optr[count + SCRW + 1]) >> 2) - nptr[count];
	...
	for (cy = top; cy < bottom; cy++) {
		for (cx = left; cx < right; cx++) {
			...
			if (square < radsquare) {
				...
				Height[page][SCRW * (cy + y) + cx + x] += (int)((FCos(dist) + 0xffff) * (height)) >> 19;
			}
		}
	}
}
