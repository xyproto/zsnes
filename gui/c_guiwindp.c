...
void DisplayNetOptns(void)
{
    ...
    GUIwinsizex[8] = 220;
    GUIwinsizey[8] = 96;
    /* Keep the window on-screen: the original default x=64 was sized for a
     * 128-wide window. With 220 width, x+width=284 would clip off a 256-wide
     * SNES-native viewport, hiding the close button. Clamp the live position
     * so it stays reachable regardless of any saved or dragged value. */
    if ((s4)GUIwinposx[8] + (s4)GUIwinsizex[8] > 254)
        GUIwinposx[8] = 254 - GUIwinsizex[8];
    if ((s4)GUIwinposx[8] < 2)
        GUIwinposx[8] = 2;
...
#ifdef __UNIXSDL__
    GUIDisplayTextG(8, 50, 16, NetplayStatusText());
#else
    GUIDisplayTextG(8, 50, 16, "UNSUPPORTED ON THIS PORT");
#endif
    GUIDisplayText(8, 6, 28, "SERVER:");
    GUIDisplayBBox(8, 46, 24, 214, 34, 167);
    GUIOuttextwin2d(8, 48, 28, NetplayHostName, 26, GUINetplayTextPtr, 0);
    GUIDisplayText(8, 6, 46, "FORMAT: host[:port][/room]");
    GUIDisplayText(8, 6, 56, "PORT 7845 - TLS RELAY");
    GUIDisplayCheckbox(8, 8, 66, &NetplayTLSConfig, "USE TLS");

    DrawGUIButton(8, 8, 74, 104, 85, "CONNECT", 86, 0, 0);
    DrawGUIButton(8, 114, 74, 212, 85, "DISCONNECT", 87, 0, 0);
}
