...
int ConnectServer(char* servername, unsigned int port)
{
    ...
    if (UDPEnable) {
        ...
        if (bind(userversocket, (struct sockaddr*)&userveraddress, sizeof(userveraddress)) == SOCKET_ERROR) {
#ifdef __UNIXSDL__
            STUB_FUNCTION;
#else
            tcperr = WSAGetLastError();
...
int PacketReceive()
{
    ...
    if ((ConnectAddr != 0) && (ConnectAddr != userveraddress.sin_addr.s_addr))
        return (0);
    if (retval == SOCKET_ERROR) {
        closesocket(ugamesocket);
        return (-1);
