...
extern uint32_t CRC32, cur_zst_size;
...
#define debug_input              \
	print_bin(JoyAOrig, &useda); \
	printf(" ");                 \
	print_bin(JoyBOrig, &usedb); \
	printf(" ");                 \
	print_bin(JoyCOrig, &usedc); \
	printf(" ");                 \
	print_bin(JoyDOrig, &usedd); \
	printf(" ");                 \
	print_bin(JoyEOrig, &usede); \
	printf("\n");
...
#define debug_input_used      \
	printf("Used: ");         \
	print_bin(useda, &useda); \
	printf(" ");              \
	print_bin(usedb, &usedb); \
	printf(" ");              \
	print_bin(usedc, &usedc); \
	printf(" ");              \
	print_bin(usedd, &usedd); \
	printf(" ");              \
	print_bin(usede, &usede); \
	printf("\n");
...
enum zmv_start_methods { zmv_sm_zst,
						 ...
						 zmv_sm_reset,
						 zmv_sm_clear_all };
enum zmv_video_modes { zmv_vm_ntsc,
					   zmv_vm_pal };
...
static bool zmv_create(char *filename) {
	...
	if ((zmv_vars.fp = fopen_dir(ZMoviePath, filename, "w+b"))) {
		...
		zmv_vars.header.zsnes_version = INTERNALVERSIONNUMBER & 0xFFFF;
		...
		zmv_vars.header.initial_input = (pl1contrl ? BIT(0xF) : 0) | (pl2contrl ? BIT(0xE) : 0) | (pl3contrl ? BIT(0xD) : 0) | (pl4contrl ? BIT(0xC) : 0) | (pl5contrl ? BIT(0xB) : 0) | (IS_MOUSE_1() ? BIT(0xA) : 0) | (IS_MOUSE_2() ? BIT(0x9) : 0) | (IS_SCOPE() ? BIT(0x8) : 0);
...
static void record_pad(uint8_t pad, uint8_t *flag, uint8_t *buffer, size_t *skip_bits) {
	...
	if ((current_state != *last_state) || ((zmv_vars.inputs_enabled & BIT(0x8)) && ((zmv_vars.last_joy_state.latchx != latchx) || (zmv_vars.last_joy_state.latchy != latchy)))) {
...
static void zmv_record(bool pause, uint8_t combos_used, uint8_t slow) {
	...
	if (!average) {
		average = (float)zmv_vars.header.average_fps / ((romispal) ? 250.0f : 240.0f);
	}
	...
	average = (average * (zmv_vars.header.frames + zmv_vars.header.removed_frames) + 1.0f / (float)(slow + 1)) / (float)(zmv_vars.header.frames + zmv_vars.header.removed_frames + 1);
...
static bool zmv_open(char *filename) {
	...
	if (zmv_vars.fp && zmv_header_read(&zmv_vars.header, zmv_vars.fp) && !strncmp(zmv_vars.header.magic, "ZMV", 3)) {
		uint_fast16_t i;
		size_t filename_len = strlen(filename);
		...
		if (zmv_vars.header.zsnes_version != (INTERNALVERSIONNUMBER & 0xFFFF)) {
...
#define PICK_HELP(var)                               \
	if (!strncmp(str, "$" #var, strlen(#var) + 1)) { \
		*len = strlen(#var) + 1;                     \
		return (var);                                \
	}
...
static char *encode_command_custom(uint8_t pass) {
	char *p;
	if (pass == md_custom_passes) {
		p = md_custom_last_pass;
	} else if (pass == 1) {
		p = md_custom_first_pass;
	} else {
		p = md_custom_middle_passes;
	}
...
static void raw_video_close() {
	...
	if ((MovieVideoMode != 5) && (audio_and_video && MovieVideoAudio)) {
		if (mencoderExists) {
			system_dir(ZCfgPath, encode_command(md_merge));
		}
		remove_dir(ZCfgPath, MovieAudioCompress ? md_compressed_audio : md_pcm_audio);
...
static void raw_audio_write(uint32_t samples) {
	...
	for (d_end = DSPBuffer + samples; d < d_end; d++) {
		if ((unsigned int)(*d + 0x7FFF) < 0xFFFF) {
			fwrite2((short)*d, raw_vid.ap);
			continue;
		}
		if (*d > 0x7FFF) {
			fwrite2(0x7FFF, raw_vid.ap);
		} else {
			fwrite2(0x8001, raw_vid.ap);
		}
	}
}
	...
	if (raw_vid.vp) {
		...
		for (y = 0; y < RAW_HEIGHT; y++) {
			for (x = 0; x < RAW_WIDTH; x++) {
				...
				fwrite3((((pixel & 0xF800) << 8) + ((pixel & 0xE000) << 3)) | (((pixel & 0x07E0) << 5) + ((pixel & 0x0600) >> 1)) | (((pixel & 0x001F) << 3) + ((pixel & 0x001C) >> 2)), raw_vid.vp);
			}
		}
	}
	...
	if (MovieSub.fp) {
		...
		if (frame_count > MovieSub.message_start + MovieSub.message_duration) {
			...
			do {
				...
				if (!(num = strtok(MovieSub.linebuf, ":"))) {
					return (0);
				}
				for (i = num; *i; i++) {
					if (!isdigit(*i)) {
...
void MovieInsertChapter(void) {
	switch (MovieProcessing) {
	case MOVIE_PLAYBACK:
	case MOVIE_DUMPING_NEW: // replaying - external
		zmv_add_chapter();
...
void MovieDumpRaw(void) {
	...
	if (!MovieProcessing) {
		MoviePlay();
		if ((MovieVideoMode == 5) && !movie_current_pass) {
			movie_current_pass = 1;
		}
		if (MovieForcedLengthEnabled == 2) {
			MovieForcedLength = 1;
		}
